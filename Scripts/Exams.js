    ...
    function loadCSV(file) {
        Papa.parse(file, {
            ...
            complete: function(results) {
                ...
                document.getElementById("resultRow4").innerHTML = "";
                document.getElementById("resultRow5").innerHTML = "";
            }
        });
    }
    ...
    function populateFilters() {
        const classes = new Set(), codes = new Set();
        csvData.forEach(r => {
        ...
        const fillSelect = (id, items) => {
            ...
            Array.from(items).sort().forEach(item => el.innerHTML += `<option value="${item}">${item}</option>`);
        };
        ...
        document.getElementById("class2").addEventListener("change", function() {
            updateSubjectDropdown(this.value, "subject2");
        });

        document.getElementById("class5").addEventListener("change", function() {
            updateSubjectDropdown(this.value, "subject5");
        });
    }

    function updateSubjectDropdown(selectedClass, subjectElementId) {
        const subjectSelect = document.getElementById(subjectElementId);
        subjectSelect.innerHTML = `<option value="">Select Subject</option>`;
        if (!selectedClass) return;
        const filteredSubjects = new Set();
        csvData.forEach(r => {
            if (r.Class === selectedClass && r.Subject) filteredSubjects.add(r.Subject);
        });
        Array.from(filteredSubjects).sort().forEach(sub => {
    ...
    function searchByCode() {
        ...
        if(results.length === 0) { container.innerHTML = `<div class="py-4 text-slate-400 italic">No Records Found</div>`; return; }
        const r = results[0];
        container.innerHTML = `<div class="p-6 bg-[#ffd84d] text-slate-800 rounded-xl max-w-md mx-auto mt-4 text-left"><h3 class="font-bold border-none p-0 mb-2">${r.Subject}</h3><div class="mb-2 text-sm">Paper: <b>${r['Paper Name']}</b></div><div>Code: <b>${r['Paper Code']}</b></div><div>Date: <b>${r.Date} (${r.Day})</b></div><div>Class: <b>${r.Class}</b></div><div class="mt-4 bg-black/10 inline-block px-3 py-1 rounded font-bold">Shift: ${r.Shift}</div></div>`;
    }
